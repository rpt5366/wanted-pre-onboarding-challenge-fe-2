...
안녕하세요, 이번 챌린지에 참가 신청을 한 형섭이라 합니다. 최근 프론트에서 시각적인 부분을 개발하는 것이 재미있어 신청하게 되었습니다. 학부 시절 웹 기본(HTML, CSS, Javascript)을 학습하고, 팀프로젝트로 WebGL API를 사용하여 간단한 웹 비행 게임 어플리케이션 구현에 참여한 경험도 있습니다. 최근엔 React를 틈틈이 공부하고 있습니다. 근래에 Typescript가 피할 수 없는 하나의 대세이고, 점차 많은 코드들이 Typescript로 작성되고 있다는 점, 많은 구인공고에서 Typescript를 요한다는 점에서 Typescript 공부의 필요성을 느꼈습니다. 이번 챌린지를 통해 Typescript를 배우고 커리어로 이어지면 좋겠습니다 :) 
